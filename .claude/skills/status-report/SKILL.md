...
4. **Where that puts us in the plan.** Name the chapter or item from `todo.md` and say whether it
   moved. **Read it before writing this, do not recall it**: items get ticked, added and removed by
   either of us and the list changes under you. If nothing moved, "hoofdstuk 1 staat waar het stond,
   dit was onderhoud" is a complete and useful answer. Where a claim is about how much of the format
   reads, that number lives in `docs/status.md`.
...
* Does it say where we are in the plan, checked against `todo.md` rather than remembered?
