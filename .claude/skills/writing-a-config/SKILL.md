...
* **A frame can be written and its tail cannot simply be copied**, section 152. Five durations off a
  ...
  22<!--fact:protocol_tails_stated--> entries carry a block derived from Logitech's own statement of it**, section 228, so
  51 of the table's 600<!--fact:protocol_entries--> entries can emit a whole record and the rest can emit only a frame.
