...
`docs/how-a-harmony-works.md` is the long form and `.Codex/skills/how-a-harmony-works/SKILL.md` is the
...
9. **`docs/findings.md` stays one file.** Splitting it is the obvious idea at 14957 lines and it was
   ...
   it is never loaded whole, only grepped and read in ranges; the per-session cost was `AGENTS.md`
...
That directory has its own `AGENTS.md`. Analysis happens there, only shareable output lands here.
...
reference/silhouettes/          the front face of a model, one SVG per model, **generated** from
                                ...
                                drop. `.Codex/skills/draw-remote/SKILL.md` is the method.
...
.agents/skills/                 the project skills, as relative symlinks into .Codex/skills/, so a
                                ...
                                are not, since those are Codex's own plugin mechanism and point
                                into node_modules
...
packages/codec/                 TS: the one config codec, container through compiler,
                                ...
                                Thirty eight entries, six of them measured on both routes, and each
                                ...
                                **A table entry says where its durations came from**: three off the
                                corpus, twenty nine off a configuration Logitech's own compiler produced, and
...
`.Codex/skills/ts-lsp/` and `.Codex/skills/py-lsp/` are plugin shaped directories whose `.lsp.json`
...
them is what is configured here. `enabledPlugins` in `.Codex/settings.json` is empty as a result: an
...
* **A closure whose two ends come from the same bytes is not a closure**, and it will hold anyway.
  ...
  test read neither end, `AGENTS.md` verification standard.
