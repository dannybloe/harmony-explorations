...
.PHONY: help test test-verbose lint prose ghidra clean

help:
	@echo "test         run the test suite (set HARMONY_LAB to include image-backed tests)"
	...
	@echo "ghidra       build or refresh the Ghidra project (needs HARMONY_LAB)"

test:
	@$(PYTHON) -m unittest discover -s $(TESTS)
	@if [ -z "$$HARMONY_LAB" ]; then \
	  echo; echo "note: HARMONY_LAB is unset, so image-backed tests were skipped."; fi
