...
| `0xFF` `+0xE000` | 634 | an image, version 3.4, checksum `0xD9E9`, opening with a run of `BRA`, so a jump table into a callable library. **It is the external flash programmer**, section 191: five gates at `0x01E00C` to `0x01E014`, the AMD command set unlock at `0x022AAA` and `0x022555`, erase and program on separate gates, plus a sleep routine that disables the external bus. It is resident because its callers cannot execute from the bus while they run | own checksum verifies; **this is version block field 8** |
