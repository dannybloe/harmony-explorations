...
| `AGENTS.md`, if it exists | it does not today, and that is a **decision rather than an accident**: it was removed so there would not be a second copy of `CLAUDE.md`, per this project's oldest rule. The effect is that Codex has no project brief, which is what makes job 1 blind for free. The entry is here so that writing one later does not silently open the door |
