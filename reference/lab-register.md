...
| `software/classic/res/` | resources: strings, images, layouts | 287 | `surveyed`, one file mined | `fh-screens` `fh-limits` `models` | where the interface's own vocabulary and limits would be, if anywhere. `software/classic/res/client/device.properties` is **mined** as section 207: it maps each platform codename to its skins, and following it into the unit factories gave the three transports and narrowed `isHarmony`. What is still unopened is 96 string bundles in sixteen languages and 66 HTML layouts, which is the `fh-screens` and `fh-limits` half |
