...
| `work/myharmony/model/` | **ours**: the platform's data model, extracted and checked | 5 | `catalogued` 30 August 2026 | `fh-data-model` `service-api` | **section 218.** The generated proxy's data contracts parsed into a model, 470 entities and 366 references over 38 areas, with `MODEL.md`, a core diagram and an area listing beside it. **Checked against replies the live service gave us**: Account 21 of 21 fields, Activity 25 of 25, Device 32 of 32, Remote 32 of 32, nothing in the schema missing from live data. The service also returns fields this client build does not know, so it has moved on since. **The vendor's type and field names are in here and have not been published**, pending Danny's call on the decision 2 boundary |
