...
| `AGPL for the product is deliberate` | 12 August 2026 | reversed while FreeHarmony was still a placeholder. Its own README promises no network at all, so the network clause guards a case the product has excluded, and the Affero variant is a one way door with concordance and harmony-decompiler: their GPLv3 code can come here and nothing of ours could go back. The product is GPLv3; if a shared server is ever built, the clause is that server's |
